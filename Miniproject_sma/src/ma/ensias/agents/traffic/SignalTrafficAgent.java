...
public class SignalTrafficAgent extends GuiAgent {
	...
	protected void setup() {
		...
		if (args.length == 2) {
			...
			//Get the signalTraffic information (random i ,getSignalLocList(i))
			...
			System.out.println("Certains arguments manquent");
			doDelete();
		}
		...
		System.out.println("Agent signalTraffic"+"  "+ this.getLocalName()+" est prêt");
	}
	
	//Send signal to all the other SignalTrafficAgent 
		...
		for (int i = 0; i < gui.vehicleAgentNumber; i++) {
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					send(message);
					//System.out.println(message.getSender().getLocalName()+" envoi sa position");
				}
			});
		}
	}
