...
public class VehicleAgent extends GuiAgent {
	...
	public boolean isNearSig(double x, double y) {
		...
		for (List<Double> color_coord : signalMessage.values()) {
			...
			double distToSignal = Math.sqrt(Math.pow(x - signalX, 2)+ Math.pow(y - signalY, 2));
	...
	protected void setup() {
		...
		if (args.length == 2) {
			gui =(MainApplication) getArguments()[0];
			...
			if(carType==MainApplication.crazyDriverCost) {
				agentShape.setFill(Color.BLUEVIOLET);
				speed = 1.8;
			}else if (carType==MainApplication.calmDriverCost) {
			...
			System.out.println("Aucun conteneur n'existe");
			doDelete();
		}
		...
		//Receive msg from signalTrafficAgent
		addBehaviour(new CyclicBehaviour() {
			...
			public void action() {
				...
				if (msg != null) {
					...
					//Parse stringList and change its elements from string to double
					...
					Iterator listReceivers =  msg.getAllReceiver();
					while (listReceivers.hasNext()) {
						AID r = (AID)listReceivers.next();
						if(this.getAgent() != null) {
							Agent a = this.getAgent();
							if((a.getLocalName()).equals(r.getLocalName())) {
								//System.out.println(r.getLocalName()+"recoit un message de "+msg.getSender().getLocalName()+msg.getContent());
							}
						}
					}
		...
		System.out.println("Agent vehicule"+"  "+ this.getLocalName()+" est prêt."+"Type de conducteur"+type);
	}

	public void initialize() {
		//Get random point of start
		...
		if (nextTarget != null) {
			...
			Platform.runLater(new Runnable() {
				@Override
				public void run() {
					//System.out.println("aaaaaaaaaaaaaaa"+nextTarget+"       "+nextTarget.X+"     "+nextTarget.Y);
